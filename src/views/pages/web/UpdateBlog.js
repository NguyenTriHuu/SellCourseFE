...
function UpdateBlog() {
    ...
    useEffect(() => {
        axiosPrivate
            .get(`/api/blog/get/${location?.state?.idBlog}`)
    ...
    useEffect(() => {
        if (data && data.content) {
            setCategorySelect(data?.idCategory);
            setTitle(data?.title);
            const contentInHtml = JSON.parse(data?.content);
            const contentBlock = htmlToDraft(contentInHtml);
            if (contentBlock) {
    ...
    const handleClick = () => {
        ...
        axiosPrivate
            .post(`/api/blog/update/${location?.state?.idBlog}`, formData, {
                headers: {
