...
function Comment2({ lessonId }) {
    ...
    useEffect(() => {
        axiosPrivate
            .get(`/api/course/lecture/${lessonId}/comment`)
            .then((res) => setData(res?.data))
            .catch((error) => console.log(error));
    }, [lessonId, messageReceived]);
    ...
    useEffect(() => {
        if (isConnect && stompClient && stompClient.connected) {
            ...
            const newSubscription = stompClient.subscribe(`/topic/comments/${lessonId}`, onMessageReceived);
            setSubscription(newSubscription);
        }
    }, [lessonId, isConnect, stompClient]);
    ...
    const sendComment = (comment) => {
        if (stompClient && stompClient.connected) {
            stompClient?.send(`/app/comment/${lessonId}`, {}, JSON.stringify(comment));
        } else {
    ...
    const sendReply = (reply, id) => {
        if (stompClient && stompClient.connected) {
            stompClient?.send(`/app/comment/reply/${id}/${lessonId}`, {}, JSON.stringify(reply));
        } else {
            console.log('Cannot send comment because stompClient is not connected');
        }
    };
    ...
    const handleDeleteComment = (e, idComment) => {
        e.stopPropagation();
        if (stompClient && stompClient.connected) {
            stompClient?.send(`/app/comment/${idComment}/${lessonId}/delete`, {});
        } else {
            console.log('Cannot send comment because stompClient is not connected');
        }
    };
    ...
    const handleDeleteCommentReply = (e, idReply) => {
        e.stopPropagation();
        if (stompClient && stompClient.connected) {
            stompClient?.send(`/app/comment/reply/${idReply}/${lessonId}/delete`, {});
        } else {
            console.log('Cannot send comment because stompClient is not connected');
        }
    };
